...
//                              new Lambertian(Color(0.8, 0.8, 0.0)));
//     *(list + 2) =
//         new Sphere(Point3(1, 0, -1), 0.5, new Metal(Color(0.8, 0.6, 0.2), 0.3));
...
void createWorld(Hittable **list, Hittable **world, Camera **camera, int widht,
    ...
    *(list) = new Sphere(Point3(0,-1000.0,-1), 1000,
        ...
        for(int b = -11; b < 11; b++) {
            ...
            if(choose_mat < 0.8f) {
                list[i++] = new Sphere(center, 0.2,
                                             new Lambertian(Color(randf()*randf(),
                                                                  randf()*randf(),
                                                                  randf()*randf())));
            }
            else if(choose_mat < 0.95f) {
                list[i++] = new Sphere(center, 0.2,
                                             new Metal(Color(0.5f*(1.0f+randf()), 0.5f*(1.0f+randf()), 0.5f*(1.0f+randf())), 0.5f*randf()));
            }
            else {
    ...
    list[i++] = new Sphere(Point3(-4, 1, 0),  1.0, new Lambertian(Color(0.4, 0.2, 0.1)));
    list[i++] = new Sphere(Point3(4, 1, 0), 1.0, new Metal(Color(0.7, 0.6, 0.5), 0.0));
